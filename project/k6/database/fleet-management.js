...
export function clearFleetData() {
  http.get(
    `http://${__ENV.SENSAE_INSTANCE_IP}:9000/exec?limit=0%2C1000&explain=true&count=true&src=con&query=TRUNCATE%20TABLE%20data&timings=true`
  );
...
export function countFleetMeasuresEntries() {
  const res = http.get(
    `http://${__ENV.SENSAE_INSTANCE_IP}:9000/exec?query=SELECT%20count%20FROM%20data`
  );
