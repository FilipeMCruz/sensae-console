...
export function countSmartIrrigationMeasuresEntries() {
  const res = http.get("http://localhost:9098/exec?query=SELECT%20count%20FROM%20smart_irrigation_data");
...
const irrigationDB = sql.open(
  "postgres",
  `postgres://admin:quest@localhost:8898/qdb?sslmode=disable`
);
